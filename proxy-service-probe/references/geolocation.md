...
6. **其余冲突**：普通非受限 Google 国家与数据库强共识冲突时，保留候选和全部口径并降置信度。仅部分数据库分歧时保留候选但降置信度。Google 地区无效时才按 GeoIP/CF 共识定国，不用旧名称兜底。
7. **未知兜底以 Google 为准**：上述仲裁后仍为 `UNK` 的节点（Google 与数据库冲突、数据库无共识、跨出口国家冲突、出口未验证），最终一律按 Google 观测地区定国：Gemini 显式地区优先于 YouTube，最近一次观测优先（地区变化时取复查结果）。`geo_decision.basis` 记为 `google_fallback_gemini` / `google_fallback_youtube`，原未知原因保留在 `unknown_basis`，置信度降为 low 并标待复核。Gemini 与 YouTube 都没有显式地区时才保留未知。
8. **多出口**：同族多个 IP 才是疑似轮换池，IPv4+IPv6 本身不是。跨出口国家冲突或复核失败不可报告稳定。Google 请求无法直接回显源 IP，报告不能声称已证明其目的站出口与回显站完全相同。
