...
Use `source-coverage.md` when a finding needs book-level attribution, exceptions, or tradeoffs.
...
> - "History Tracking" — only when persistent score history was requested
...
In a multi-mode session, reuse unchanged config; re-read when it or the relevant workspace changes.
...
**`sweep`** — Full Sweep only. `max_iterations` (default 3) is a progress-review
budget. Reassess evidence and authorization at that point, report unresolved findings
honestly, and never classify capped or retired issues as fixed. Other modes ignore it.
...
If the YAML fails to parse, report the error. Continue only the analysis independent of the ambiguous settings; do not silently disregard exclusions or claim the config was applied.
...
**PR Review:** inspect staged and unstaged changes; if neither exists, compare against the actual project base branch. Ask only when the intended range cannot be determined. Do not assume the branch is named main.
...
<!-- classDef colors: see ../brooks-audit/architecture-guide.md Step 1 Rule 6 -->
...
When the user requests remediation guidance, read the relevant parts of
`remedy-guide.md`. Diagnostic requests remain read-only. An explicit request to
apply fixes authorizes the scoped local repair flow in `../brooks-sweep/sweep-guide.md`;
do not ask again for that same authorization.
...
Only if the user requests persistent score history, append a record to
`.brooks-lint-history.json` in the project root. Read-only audits do not create or modify it.
...
**Guard:** Only when the user requests interactive triage; otherwise end with the requested report.
...
| File | When to Read |
|------|-------------|
| `source-coverage.md` | Book-level attribution, exceptions or tradeoffs are needed |
| `decay-risks.md` | A production-code, architecture or debt risk needs classification |
| `test-decay-risks.md` | A test-quality risk needs classification |
